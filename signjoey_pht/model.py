...
class SignModel(nn.Module):
    ...
    def get_loss_for_batch(
        ...
        if self.sim_loss_weight > 0:
            ...
            cos_sims = torch.mm(sentence_emb, sentence_emb.t())   #############手语编码余弦相似度
            #print('cos_sim')
            sim_labels = sentence_emb.new_zeros((batch_size, batch_size))
            sim_mask = torch.triu(                                ###########上三角，对角线及其以上的元素都为1
                sentence_emb.new_ones((batch_size, batch_size), dtype=bool), 1
            )
            index_list = []
            for i in range(batch_size - 1):                    ###########文本相似度
                for j in range(i + 1, batch_size):
                    sim_labels[i, j] = self.video_cos_sim[
                        self.name_to_video_id[batch.sequence[i]],    ###查找对应的文本相似度，用setence-bert离线计算过的
                        self.name_to_video_id[batch.sequence[j]],    ###sequence应该是是视频名称
                    ]
                    index_list.append(self.name_to_video_id[batch.sequence[i]])
            ...
            sim_loss = sim_loss * self.sim_loss_weight + clip_loss * self.clip_loss_weight 
