...
from SLTFA.initialization import initialize_model
from SLTFA.embeddings import Embeddings, SpatialEmbeddings
from SLTFA.encoders import (
    Encoder,
    RecurrentEncoder,
    TransformerEncoder,
    SepViTEncoder,
...
from SLTFA.decoders import (
    Decoder,
    RecurrentDecoder,
    TransformerDecoder,
    CrossTransformerDecoder,
)
from SLTFA.search import beam_search, greedy
from SLTFA.vocabulary import (
    TextVocabulary,
    GlossVocabulary,
    PAD_TOKEN,
    EOS_TOKEN,
...
from SLTFA.batch import Batch
from SLTFA.helpers import freeze_params, get_mask_from_sequence_lengths
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        gloss_rate: int = 10,    ######gloss_rate表示的是什么
        sim_loss_weight=0.0,
    ...
    def forward(
        ...
        if self.query_embedding is not None:
            ...
            else:                                      ######默认gloss_rate=10，query的长度是手语长度//10
                query_lens = sgn_lengths // self.gloss_rate

            query_lens = torch.where(                #####对于所有小于 1 的 query_lens，将其替换为 1，以确保长度不小于 1
    ...
    def get_loss_for_batch(
        ...
        if self.clip_loss_weight > 0:
            ...
            if self.sentence_embedding_mod == "mean":          ##############默认是平均
            ...
            #sentence_emb = F.normalize(sentence_emb, dim=1)
            selected_index_list = []
            for i in range(batch_size):                    ###########文本相似度
                selected_index_list.append(self.name_to_video_id[batch.sequence[i]])   ###查找对应的文本相似度，用setence-bert离线计算过的
            #print('selected_index_list:',selected_index_list)
            text_emb = self.text_emb[selected_index_list]
            text_emb = F.normalize(text_emb, dim=1).cuda()
            #print('text_emb:',text_emb.shape)
            #print('text:',text_emb.device)   cpu
            #print('sentence:',sentence_emb.device)  cuda
            logit_scale = self.logit_scale.exp()
            logits_per_video = logit_scale * sentence_emb @ text_emb.t() 
            logits_per_text = logits_per_video.t()     
            labels = torch.arange(logits_per_video.shape[0], dtype=torch.long).cuda()
            clip_loss = (
            ...
            # sim_loss = sim_loss * (self.sim_loss_weight / 2) + clip_loss * (self.sim_loss_weight / 2)
            sim_loss = clip_loss * self.clip_loss_weight 
    ...
    def run_batch(
        ...
        query_output = None
        query_mask = None
        #print('self.query_embedding is None: ',self.query_embedding is None)
        if self.query_embedding is not None:
            if self.gloss_rate < 0:
...
def build_model(
    ...
    if cfg["encoder"].get("type", "recurrent") == "SepViT_transformer":
        assert (
    ...
    sim_text_emb = cfg.get("sim_text_emb","")
    text_emb = pickle.load(
        open(sim_text_emb, "rb")
    )
    ...
    model: SignModel = SignModel(
        ...
        clip_loss_weight=cfg.get("clip_loss_weight", 0.0),
